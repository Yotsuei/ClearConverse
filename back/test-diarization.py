...
def test_diarization_thresholds(audio_files, thresholds_to_test, auth_token, output_dir="threshold_tests"):
    """
    Test various threshold combinations on audio files.
    
    Args:
        audio_files: List of paths to audio files to test
        thresholds_to_test: List of (embedding_threshold, secondary_threshold) tuples
    ...
    with open(csv_path, "w", newline='') as csvfile:
        ...
        writer.writerow([
            "Audio File", "Embedding Threshold", "Secondary Threshold", 
            "Speaker Attribution Score (1-5)", "Overlap Handling Score (1-5)",
            "False Speaker Changes", "Missed Speaker Changes", "Notes"
        ])
        
        for audio_file in audio_files:
            base_name = os.path.basename(audio_file)
            for emb_threshold, sec_threshold in thresholds_to_test:
                writer.writerow([
                    base_name, emb_threshold, sec_threshold,
                    "", "", "", "", ""  # Empty fields for manual evaluation
                ])
    
    # Process each file with each threshold combination
    results = {}
    ...
    for audio_file in audio_files:
        ...
        for emb_threshold, sec_threshold in thresholds_to_test:
            logging.info(f"Testing thresholds: embedding={emb_threshold}, secondary={sec_threshold}")
            
            # Create unique output directory for this test
            test_name = f"{base_name}__emb{emb_threshold}__sec{sec_threshold}".replace(".", "_")
            test_output_dir = os.path.join(output_dir, test_name)
            os.makedirs(test_output_dir, exist_ok=True)
            
            # Create custom config with these thresholds
            config = Config(
                auth_token=auth_token,
                speaker_embedding_threshold=emb_threshold,
                secondary_diarization_threshold=sec_threshold
            )
            
            # Process the file with these settings
            processor = EnhancedAudioProcessor(config, load_models_immediately=True)
            ...
            try:
                ...
                # Store result
                file_results[(emb_threshold, sec_threshold)] = {
                    "success": True,
                ...
                logging.error(f"Error processing with thresholds {(emb_threshold, sec_threshold)}: {e}")
                file_results[(emb_threshold, sec_threshold)] = {
                    "success": False,
                    "error": str(e),
                    "output_dir": test_output_dir
    ...
    with open(os.path.join(output_dir, "results_summary.json"), "w") as f:
        ...
        for file_name, file_results in results.items():
            summary[file_name] = {}
            for thresholds, result in file_results.items():
                emb, sec = thresholds
                summary[file_name][f"emb{emb}_sec{sec}"] = {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test Speaker Diarization Thresholds')
    parser.add_argument('--audio_dir', type=str, required=True, help='Directory containing audio files to test')
    parser.add_argument('--output_dir', type=str, default='threshold_tests', help='Directory to store test results')
    ...
    parser.add_argument('--file_extensions', type=str, default='.mp3,.wav', help='Comma-separated list of file extensions to process')
    ...
    thresholds_to_test = [
        # (speaker_embedding_threshold, secondary_diarization_threshold)
        (0.30, 0.25), (0.30, 0.30), (0.30, 0.35),
        (0.35, 0.25), (0.35, 0.30), (0.35, 0.35),
        (0.40, 0.25), (0.40, 0.30), (0.40, 0.35),
        (0.45, 0.25), (0.45, 0.30), (0.45, 0.35),
        (0.50, 0.25), (0.50, 0.30), (0.50, 0.35),
    ]
    ...
    for ext in extensions:
        audio_files.extend(list(Path(args.audio_dir).glob(f"*{ext}")))
    ...
    confirm = input(f"\nThis will test {len(thresholds_to_test)} threshold combinations on {len(audio_files)} files. Continue? (y/n): ")
    if confirm.lower() != 'y':
        print("Operation cancelled")
        exit(0)
    
    # Run the tests
    results = test_diarization_thresholds(
